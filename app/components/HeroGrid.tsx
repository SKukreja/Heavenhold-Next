...
const HeroLink = ({ hero }: { hero: Hero }) => {
  ...
  return (
    <Link
      ...
      className={`${"e-" + bioFields?.element?.toLowerCase()} ${bioFields?.element?.toLowerCase()} r-${bioFields?.rarity?.toString().replace(/ /g, "-").toLowerCase()} ${bioFields?.role?.toLowerCase()} relative w-[calc(50vw-2.5rem-1px)] h-[calc((50vw-2rem)*3/2)] lg:w-[calc((85vw*0.25)-2rem)] lg:h-[calc(((85vw*0.25)-2rem)*3/2)] 2xl:w-[calc((85vw*0.20)-2rem)] 2xl:h-[calc(((85vw*0.20)-2rem)*3/2)] 3xl:w-[calc((85vw*0.15)-2rem)] 3xl:h-[calc(((85vw*0.15)-2rem)*3/2)] flex m-0 lg:m-[10px] cursor-pointer align-middle transition-all duration-200 after:transition-all after:linear after:duration-200 hover:after:outline-offset-[-5px] ease grayscale-[30%] hover:grayscale-0 after:w-full after:h-full after:absolute after:inset-0 after:z-20 after:pointer-events-none after:border after:border-gray-800 after:outline after:outline-2 after:outline-offset-[-10px]`}
    >
