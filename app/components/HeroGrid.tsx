            ...
            + ' relative w-64 h-96 flex m-[10px] cursor-pointer align-middle transition-all duration-200 after:transition-all after:linear after:duration-200 hover:after:outline-offset-[-5px] ease grayscale-[30%] hover:grayscale-0 after:w-full after:h-full after:absolute after:inset-0 after:z-20 after:pointer-events-none after:border after:border-[#2e2923] after:outline after:outline-2 after:outline-[#6c5223] after:outline-offset-[-10px]'}>
                    ...
                    <div className="absolute top-0 left-0 right-0 flex justify-between p-4 pointer-events-none h-36 align-center before:bg-gradient-to-b before:from-black before:to-transparent before:w-full before:h-full before:z-10 before:absolute before:opacity-50 before:inset-0">
                        ...
                        <span className="relative z-20 text-lg font-bold text-center w-100 drop-shadow-2xl">
                            {hero.heroInformation?.abilityFields?.chainStateTrigger && hero.heroInformation?.abilityFields?.chainStateTrigger != "None" && (<Image src={'/icons/' + hero.heroInformation?.abilityFields.chainStateTrigger.toString().toLowerCase() + '.webp'} className="mb-2" width={25} height={25} alt={hero.heroInformation?.abilityFields?.chainStateTrigger} />)}
                            {hero.heroInformation?.abilityFields?.chainStateResult && hero.heroInformation?.abilityFields?.chainStateResult != "None" &&(<Image src={'/icons/' + hero.heroInformation?.abilityFields.chainStateResult.toString().toLowerCase() + '.webp'} className="mb-2" width={25} height={25} alt={hero.heroInformation?.abilityFields?.chainStateResult} />)}
                        </span>
                    </div>
