...
export default function TeamsList({ hero, item, activeCategory }: TeamsListProps) {
  ...
  const { data: voteData } = useGetUserTeamVoteStatusSuspenseQuery({
    variables: { userId: user?.user_id ?? 0, ipAddress: ipAddress.toString() },
  });
  ...
  // Compute filteredTeams using useMemo
  const filteredTeams = useMemo(() => {
    ...
    if (activeCategory && hero) {
      return teams
        .filter(
          (team) =>
            team.teamFields?.teamType?.toLowerCase().replace(/ /g, "-") ===
            activeCategory.replace(/ /g, "-")
        )
        .filter((team) =>
          team.teamFields?.composition?.some((slot) => slot?.hero?.nodes[0].id === hero.id)
        ) as Team[];
  ...
  }, [teams, activeCategory, hero, item]);

  // Local state to manage user votes
  const [localVotes, setLocalVotes] = useState<{
    [teamId: number]: { votes: number; userVote: string | null };
  }>({});

  // Compute combinedTeams using useMemo
  const combinedTeams = useMemo(() => {
    ...
    const teamsWithVotes = filteredTeams
      .map((team) => {
        ...
        const votesCount = (vote?.upvoteCount ?? 0) - (vote?.downvoteCount ?? 0);
        const userVoteFromData =
          voteData?.getUserTeamVoteStatus?.find((userVote) => userVote?.teamId === team.databaseId)
            ?.userVote ?? null;
        const localVote = localVotes[team.databaseId];

        return {
          ...team,
          votes: localVote?.votes ?? votesCount,
          userVote: localVote?.userVote ?? userVoteFromData,
        };
      })
    ...
    const teamsWithoutVotes = filteredTeams
      .filter(
        (team) => !votes?.getTeamVotes?.some((vote) => vote?.teamId === team.databaseId)
      )
      .map((team) => ({ ...team, votes: 0, userVote: null }));

    return [...teamsWithVotes, ...teamsWithoutVotes];
  }, [filteredTeams, votes, voteData, localVotes]);
    ...
    try {
      ...
      if (data?.upvoteTeam?.success) {
        setLocalVotes((prevVotes) => {
          const prevVote = prevVotes[teamId] || {
            votes: combinedTeams.find((t) => t.databaseId === teamId)?.votes || 0,
            userVote: null,
          };
          let newVotes = prevVote.votes;
          let newUserVote: string | null = "upvote";

          if (prevVote.userVote === "downvote") {
            newVotes += 2;
          } else if (prevVote.userVote === "upvote") {
            newVotes -= 1;
            newUserVote = null;
          } else {
            newVotes += 1;
    ...
    try {
      ...
      if (data?.downvoteTeam?.success) {
        setLocalVotes((prevVotes) => {
          ...
          let newVotes = prevVote.votes;
          let newUserVote: string | null = "downvote";

          if (prevVote.userVote === "upvote") {
            newVotes -= 2;
          } else if (prevVote.userVote === "downvote") {
            newVotes += 1;
            newUserVote = null;
          } else {
            newVotes -= 1;
  ...
  const toggleTeamDetails = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    if (
    ...
    <Suspense fallback={<Loading />}>
      {combinedTeams.length > 0 ? (
                  ...
                  <div className="flex justify-between items-center gap-2 lg:gap-4 flex-1">
                    <div className="votes flex flex-col justify-between items-center gap-2">
                      <button
                        ...
                        onClick={(e) => handleUpvote(team?.databaseId ?? 0, e)}
                        disabled={
                          team && "userVote" in team && team.userVote === "upvote"
                        }
                      >
                        {upvote()}
                      </button>
                      ...
                      <button
                        ...
                        onClick={(e) => handleDownvote(team?.databaseId ?? 0, e)}
                        disabled={
                          team && "userVote" in team && team.userVote === "downvote"
                        }
                      >
                        {downvote()}
                      </button>
                ...
                <div className="flex-col-reverse lg:flex-row flex w-4/5">
                  <div className="team-explanation flex-1 flex flex-col gap-4 pr-0 lg:pr-32 w-full lg:w-[calc(50%-3rem)]">
                    ...
                    {combinedPartyBuffArray.length > 0 && (
                      <div className="team-party-buffs mt-4">
                        <h2 className="mb-2 text-lg font-medium font-oswald">
                          Team Passives
                        </h2>
                  ...
                  <div className="team-build flex items-start justify-start gap-2 lg:gap-4 -ml-[calc(3rem)] w-[calc(100%+3rem)] lg:w-[calc(50%+3rem)]">
                    <div className="w-32 h-full flex flex-col justify-start items-center gap-4">
                      ...
                      <div className="w-8 h-16 xl:h-12 2xl:h-12 flex justify-center items-center text-center pr-4">
                        <FadeInImage
                          className={"w-4 h-auto"}
                          src={equipmentIcons["One-Handed Sword"]}
                          width={12}
                          height={12}
                          alt={"Weapon icon"}
                        />
                      ...
                      <div className="w-8 h-16 xl:h-12 2xl:h-12 flex justify-center items-center text-center pr-4">
                        <FadeInImage
                          className={"w-4 h-auto pl-[calc(0.1rem)] pr-[calc(0.1rem)]"}
                          src={equipmentIcons["Cards"]}
                          width={12}
                          height={12}
                          alt={"Card icon"}
                        />
                    ...
                    {team.teamFields?.composition?.map((slot: any, index: number) => {
                      const heroId = slot?.hero?.nodes[0].id;
                      const heroData = heroes.find((h) => h.id === heroId);
                      const element =
                        heroData?.heroInformation?.bioFields?.element?.toLowerCase();
                          ...
                          <div className="build-items">
                            <div className="build-object build-weapon">
                              <div className="build flex flex-col gap-4 text-2xs">
                                <Link
                                  href={`${weapon?.uri}`}
                                  className={`flex items-center gap-1 lg:gap-2 h-16 xl:h-12 2xl:h-12 ${
                                    weapon ? "" : "pointer-events-none text-gray-500"
                                  }`}
                                >
                                  <FadeInImage
                                    src={
                                    ...
                                    alt={weapon?.title + " Icon"}
                                  />
                                  <span className="w-full">
                                    {weapon ? weapon?.title : "N/A"}
                                  </span>
                                </Link>
                                {/* Repeat similar blocks for shield, accessory, merch, relic, and cards */}
                                {/* ... */}
                              </div>
                            </div>
                          </div>
        ...
        <div className="flex flex-col w-full h-[calc(50vh)] justify-center items-center">
          <span>
            No teams found
            {item ? " with " + item?.title : hero ? " for " + hero?.title : ""}.
          </span>
          <Link
