                    ...
                    <div className={`flex flex-col gap-1 overflow-hidden ${selectedWeapon === index ? 'h-auto' : 'h-0'} text-xs 3xl:text-sm 4xl:text-base`}>
                      {item?.equipmentOptions?.mainStats ? (
                        item.equipmentOptions.mainStats.map((stat, index) => (
                          <StatFormatter
                            statName={stat?.stat + ""}
                            element={item?.equipmentOptions?.mainStats?.find(
                              (x) =>
                                x?.stat?.includes("Fire") ||
                                x?.stat?.includes("Earth") ||
                                x?.stat?.includes("Water")
                            )?.stat?.toString().replace(" Atk", "")}
                            statValue={
                              stat?.isRange == true
                                ? stat.minValue + " - " + stat.maxValue
                                : stat?.value?.toString() || "" // Provide a default value of an empty string
                            }
                            isPassive={false}
                          />
                        ))
